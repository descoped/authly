...
class TestRequireAdminScope:
    ...
    async def test_require_admin_scope_success(
        self,
        test_admin_user: UserModel,
        admin_token_with_all_scopes: str,
        initialize_authly: Authly
    ):
        ...
        credentials = HTTPAuthorizationCredentials(
            scheme="Bearer",
            credentials=admin_token_with_all_scopes
        )
        
        # Call the dependency
        result = await require_clients_write(
    ...
    async def test_require_admin_scope_missing_scope(
        self,
        test_admin_user: UserModel,
        admin_token_limited_scopes: str,
        initialize_authly: Authly
    ):
        ...
        credentials = HTTPAuthorizationCredentials(
            scheme="Bearer",
            credentials=admin_token_limited_scopes
        )
        
        with pytest.raises(HTTPException) as exc_info:
            await require_clients_write(
                admin_user=test_admin_user,
                credentials=credentials
            )
    ...
    async def test_require_admin_scope_regular_user_token(
        ...
        credentials = HTTPAuthorizationCredentials(
            scheme="Bearer",
            credentials=regular_user_token
        )
...
class TestTwoLayerSecurityModel:
    ...
    async def test_two_layer_security_both_checks_pass(
        ...
        scope_result = await require_clients_write(
    ...
    @pytest.mark.asyncio
    async def test_two_layer_security_first_layer_fails(
        self,
        test_regular_user: UserModel,
        regular_user_token: str
    ):

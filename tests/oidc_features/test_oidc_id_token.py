...
class TestIDTokenGenerator:
    ...
    def test_generate_id_token_basic(self, generator, test_user, test_client):
        ...
        token = generator.generate_id_token(user=test_user, client=test_client, scopes=scopes)
    ...
    def test_generate_id_token_with_nonce(self, generator, test_user, test_client):
        ...
        token = generator.generate_id_token(user=test_user, client=test_client, scopes=scopes, nonce=nonce)
    ...
    def test_generate_id_token_with_auth_time(self, generator, test_user, test_client):
        ...
        scopes = ["openid"]
        auth_time = datetime.now(UTC) - timedelta(minutes=5)

        token = generator.generate_id_token(user=test_user, client=test_client, scopes=scopes, auth_time=auth_time)
    ...
    def test_generate_id_token_with_additional_claims(self, generator, test_user, test_client):
        ...
        scopes = ["openid"]
        additional_claims = {"custom_claim": "custom_value", "another_claim": 42}

        token = generator.generate_id_token(
            user=test_user, client=test_client, scopes=scopes, additional_claims=additional_claims
        )
    ...
    def test_generate_id_token_without_openid_scope(self, generator, test_user, test_client):
        ...
        with pytest.raises(HTTPException) as exc_info:
            generator.generate_id_token(user=test_user, client=test_client, scopes=scopes)
    ...
    def test_validate_id_token_invalid_signature(self, generator, test_client):
        ...
        claims = {
            ...
            IDTokenClaims.EXP: int((datetime.now(UTC) + timedelta(minutes=15)).timestamp()),
            IDTokenClaims.IAT: int(datetime.now(UTC).timestamp()),
        }

        # Create token with wrong key but valid key ID format
        invalid_token = jwt.encode(claims, wrong_private_key_pem, algorithm="RS256", headers={"kid": "wrong_key_id"})
    ...
    def test_validate_id_token_expired(self, generator, test_client):
        ...
        expired_token = jwt.encode(
            claims, private_key_pem, algorithm=generator.algorithm, headers={"kid": signing_key.key_id}
        )
...
class TestIDTokenService:
    ...
    async def test_create_id_token(self, service, test_user, test_client):
        ...
        token = await service.create_id_token(user=test_user, client=test_client, scopes=scopes)

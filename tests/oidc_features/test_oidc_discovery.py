...
class TestOIDCDiscoveryService:
    ...
    def sample_oauth_metadata(self):
        ...
        return OAuthServerMetadata(
            ...
            token_endpoint="https://test.example.com/api/v1/oauth/token",
            revocation_endpoint="https://test.example.com/api/v1/oauth/revoke",
    ...
    async def test_get_oidc_server_metadata(self, transaction_manager: TransactionManager):
        """Test OIDC server metadata generation."""
        async with transaction_manager.transaction() as conn:
            # Create real services
            scope_repo = ScopeRepository(conn)
            oauth_discovery_service = DiscoveryService(scope_repo)
            ...
            metadata = await oidc_discovery_service.get_oidc_server_metadata(
                issuer_url="https://test.example.com", api_prefix="/api/v1"
            )
        ...
        # Verify OAuth fields are generated correctly
        assert metadata.authorization_endpoint == "https://test.example.com/api/v1/oauth/authorize"
        assert metadata.token_endpoint == "https://test.example.com/api/v1/oauth/token"
        assert metadata.revocation_endpoint == "https://test.example.com/api/v1/oauth/revoke"
        ...
        # Verify scopes list is not empty (dynamic scopes from database)
        assert len(metadata.scopes_supported) > 0
    ...
    def test_get_static_oidc_metadata(self):
        """Test static OIDC metadata generation."""
        # Create OIDC discovery service (no database needed for static metadata)
        oidc_discovery_service = OIDCDiscoveryService(None)
    ...
    async def test_oidc_metadata_with_custom_issuer(self, transaction_manager: TransactionManager):
        ...
        async with transaction_manager.transaction() as conn:
            ...
            # Generate OIDC metadata
            metadata = await oidc_discovery_service.get_oidc_server_metadata(
                issuer_url="https://auth.custom.com", api_prefix="/api/v1"
            )
...
class TestOIDCDiscoveryRouter:
    ...
    def test_get_base_url_with_forwarded_headers(self):
        ...
        request.url.netloc = "internal-server:8080"
        request.headers = {"x-forwarded-proto": "https", "x-forwarded-host": "auth.example.com"}
...
class TestOIDCDiscoveryIntegration:
    ...
    async def test_oidc_discovery_endpoint_success(self, transaction_manager: TransactionManager):
        """Test successful OIDC discovery endpoint response."""
        async with transaction_manager.transaction() as conn:
            # Create real OAuth discovery service
            scope_repo = ScopeRepository(conn)
            oauth_service = DiscoveryService(scope_repo)
            ...
            # Call endpoint
            metadata = await oidc_discovery(request, oauth_service)
    ...
    async def test_oidc_discovery_static_metadata_generation(self):
        """Test OIDC discovery static metadata generation (fallback behavior)."""
        # Test the static metadata generation directly instead of mocking database failures
        oidc_discovery_service = OIDCDiscoveryService(None)  # No OAuth service dependency

        # Generate static metadata
        metadata = oidc_discovery_service.get_static_oidc_metadata("https://test.example.com")

        # Verify static metadata response
        assert isinstance(metadata, OIDCServerMetadata)
        assert metadata.issuer == "https://test.example.com"
        assert metadata.userinfo_endpoint == "https://test.example.com/oidc/userinfo"
        assert metadata.jwks_uri == "https://test.example.com/.well-known/jwks.json"
        ...
        # Verify required OIDC compliance fields
        assert "sub" in metadata.claims_supported
        assert "name" in metadata.claims_supported
        assert "email" in metadata.claims_supported
        assert "RS256" in metadata.id_token_signing_alg_values_supported
...
class TestOIDCDiscoveryCompliance:
    ...
    def test_oidc_metadata_required_fields(self):
    ...
    def test_oidc_metadata_openid_scope_requirement(self):
    ...
    def test_oidc_metadata_subject_types(self):
    ...
    def test_oidc_metadata_signing_algorithms(self):
    ...
    def test_oidc_metadata_response_types(self):
    ...
    def test_oidc_metadata_urls_valid(self):
        ...
        assert "/oauth/token" in metadata.token_endpoint

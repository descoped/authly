...
class TestComprehensiveOIDCFlows:
    ...
    async def _create_auth_code_direct(self, db_pool, client: OAuthClientModel, user: UserModel, 
                                     scope: str = "openid profile", nonce: str = "test_nonce",
                                     max_age: int = None, prompt: str = None, 
    ...
    async def test_rs256_id_token_signing(
        ...
        auth_code = await self._create_auth_code_direct(
            db_pool, test_client_rs256, test_user,
            scope="openid profile email",
            nonce="rs256_test_nonce"
        )
    ...
    async def test_hs256_id_token_signing(
        ...
        # Create authorization code
        auth_code = await self._create_auth_code_direct(
            db_pool, test_client_hs256, test_user,
            scope="openid profile",
            nonce="algorithm_test_nonce"
        )
    ...
    async def test_comprehensive_scope_combinations(
        ...
        for scope, expected_claims in scope_combinations:
            ...
            auth_code = await self._create_auth_code_direct(
                db_pool, test_client_rs256, test_user,
                scope=scope,
                nonce=f"test_nonce_{scope.replace(' ', '_')}"
            )
    ...
    async def test_max_age_parameter_handling(
        ...
        auth_time = datetime.fromtimestamp(id_token_payload["auth_time"], timezone.utc)
    ...
    async def test_prompt_parameter_handling(
        ...
        for prompt_value in prompt_values:
            ...
            id_token_payload = jwt.decode(token_json["id_token"], key="", options={"verify_signature": False, "verify_aud": False})
            assert id_token_payload["sub"] == str(test_user.id)
    ...
    async def test_id_token_expiration_validation(
        ...
        iat = datetime.fromtimestamp(id_token_payload["iat"], timezone.utc)
        exp = datetime.fromtimestamp(id_token_payload["exp"], timezone.utc)
...
class TestOIDCErrorHandling:
    ...
    async def test_invalid_nonce_parameter(
        self, 
        oidc_server: AsyncTestServer,
        test_client: OAuthClientModel
    ):
    ...
    async def test_invalid_max_age_parameter(
        self, 
        oidc_server: AsyncTestServer,
        test_client: OAuthClientModel
    ):
    ...
    async def test_userinfo_endpoint_error_scenarios(
        self, 
        oidc_server: AsyncTestServer
    ):
    ...
    async def test_jwks_endpoint_availability(
        self, 
        oidc_server: AsyncTestServer
    ):
    ...
    async def test_discovery_endpoint_error_handling(
        self, 
        oidc_server: AsyncTestServer
    ):
...
class TestOIDCJWKSKeyRotation:
    ...
    async def test_jwks_multiple_keys_support(
        self, 
        oidc_server: AsyncTestServer
    ):
    ...
    async def test_id_token_kid_header_consistency(
        self, 
        oidc_server: AsyncTestServer,
        db_pool
    ):
        ...
        if id_token_header["alg"] == "RS256":
            ...
            assert token_kid in available_kids, f"Token kid {token_kid} not found in JWKS available kids: {available_kids}"
...
class TestOIDCAuthorizationCodeFlow:
    ...
    async def test_complete_oidc_flow_basic(
        ...
        async for db_conn in authly_db_connection():
            ...
            if stored_auth_code:
                print(f"Debug - Found auth code in DB: {stored_auth_code.code}")
                print(f"Debug - Stored client_id: {stored_auth_code.client_id}")
                print(f"Debug - Expected client_id: {test_client_confidential.id}")
                print(f"Debug - Client IDs match: {stored_auth_code.client_id == test_client_confidential.id}")
                redirect_match = stored_auth_code.redirect_uri == test_client_confidential.redirect_uris[0]
                print(f"Debug - Redirect URIs match: {redirect_match}")
                print(f"Debug - Code challenge: {stored_auth_code.code_challenge}")
                print(f"Debug - Is expired: {stored_auth_code.is_expired()}")
    ...
    async def test_oidc_flow_with_nonce_validation(
        ...
        unique_nonce = f"test_nonce_{datetime.now(timezone.utc).isoformat()}"
    ...
    async def test_oidc_refresh_token_flow(
        ...
        refresh_response = await oidc_server.client.post("/api/v1/auth/token", json=refresh_data)
...
class TestOIDCFlowIntegration:
    ...
    async def test_oidc_discovery_endpoint_consistency(
        self, 
        oidc_server: AsyncTestServer
    ):

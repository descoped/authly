...
class TestSecurePasswordGeneration:
    ...
    def test_generate_secure_password_complexity(self):
        ...
        for _ in range(10):
            ...
            assert any(c in string.ascii_uppercase for c in password), f"Password '{password}' missing uppercase"

            # Check for at least one lowercase
            assert any(c in string.ascii_lowercase for c in password), f"Password '{password}' missing lowercase"

            # Check for at least one digit
            assert any(c in string.digits for c in password), f"Password '{password}' missing digit"

            # Check for at least one special character
            special_chars = "!@#$%^&*()-_=+"
            assert any(c in special_chars for c in password), f"Password '{password}' missing special character"
    ...
    def test_generate_secure_password_valid_characters(self):
        ...
        for _ in range(10):
            password = generate_secure_password()
            assert all(c in valid_chars for c in password), f"Password '{password}' contains invalid characters"

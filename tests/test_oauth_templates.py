...
# Removed complex OAuth fixtures to focus on template infrastructure testing
    ...
    async def test_authorization_template_rendering(self, template_server: AsyncTestServer):
        ...
        assert status_code in [401, 403], f"Expected 401 or 403, got {status_code} - this means the OAuth router is working"
    ...
    async def test_css_styling_elements(self, template_server: AsyncTestServer):
        ...
        required_classes = [
            ...
            ".consent-form", ".auth-card", ".form-input", ".form-label",
            ".client-info", ".scope-item", ".security-notice", ".alert-error",
            ".btn-group", ".logo-text", ".main-content"
        ]
    ...
    async def test_template_security_features(self, template_server: AsyncTestServer):
        ...
        template_path = os.path.join(
            ...
            "src", "authly", "templates", "oauth", "authorize.html"
        )
    ...
    async def test_error_template_rendering(self, template_server: AsyncTestServer):
        ...
        error_template_path = os.path.join(
            os.path.dirname(os.path.dirname(__file__)), 
            "src", "authly", "templates", "oauth", "error.html"
        )
    ...
    async def test_template_inheritance(self, template_server: AsyncTestServer):
        ...
        base_template_path = os.path.join(
            os.path.dirname(os.path.dirname(__file__)), 
            "src", "authly", "templates", "base.html"
        )
        
        authorize_template_path = os.path.join(
            os.path.dirname(os.path.dirname(__file__)), 
            "src", "authly", "templates", "oauth", "authorize.html"
        )
        ...
        with open(authorize_template_path, 'r') as f:

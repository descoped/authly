...
class TestTokenIntrospection:
    ...
    @pytest.mark.asyncio
    async def test_introspect_active_token(self, test_server, committed_client_with_scopes):
        ...
        async with test_server.client as client:
            ...
            assert read_scope in introspect_data.get("scope", "")
            assert introspect_data["token_type"] == "Bearer"

    @pytest.mark.asyncio
    async def test_introspect_expired_token(self, test_server, committed_client_with_scopes, monkeypatch):
    ...
    @pytest.mark.asyncio
    async def test_introspect_revoked_token(self, test_server, committed_client_with_scopes):
        ...
        async with test_server.client as client:
            ...
            # Debug: print introspection result if still active
            if introspect_data.get("active"):
                print(f"Token still active after revocation: {introspect_data}")

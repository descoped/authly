...
class TestAdvancedOIDCClaims:
    ...
    async def test_phone_scope_claims(
        ...
        auth_code = await self._create_auth_code_direct(
            db_pool, test_client, enhanced_test_user, scope="openid phone", nonce="phone_scope_nonce"
        )
        ...
        jwt.decode(id_token, key="", options={"verify_signature": False, "verify_aud": False})
    ...
    async def test_address_scope_claims(
        ...
        auth_code = await self._create_auth_code_direct(
            db_pool, test_client, enhanced_test_user, scope="openid address", nonce="address_scope_nonce"
        )
...
class TestOIDCParameterHandling:
    ...
    async def test_ui_locales_parameter(self, oidc_server: AsyncTestServer, test_client: OAuthClientModel):
...
class TestOIDCTokenValidation:
    ...
    async def test_id_token_aud_claim_validation(self, oidc_server: AsyncTestServer):
...
class TestOIDCSpecCompliance:
    ...
    async def test_required_oidc_endpoints_availability(self, oidc_server: AsyncTestServer):

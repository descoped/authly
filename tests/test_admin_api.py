...
async def admin_token_with_scopes(
    ...
    async with transaction_manager.transaction() as conn:
        ...
        basic_scopes = [
            ("read", "Read access", True, True),
            ("write", "Write access", False, True)
        ]
...
class TestAdminAPIClientManagement:
    ...
    @pytest.mark.asyncio
    async def test_list_clients_with_pagination(
        self,
        test_server: AsyncTestServer,
        admin_token_with_scopes: str
    ):
        ...
        response = await test_server.client.get(
            "/admin/clients?limit=5&offset=0", 
            headers=headers
        )
        await response.expect_status(200)
    ...
    @pytest.mark.asyncio
    async def test_create_public_client_success(
        self,
        test_server: AsyncTestServer,
        admin_token_with_scopes: str
    ):

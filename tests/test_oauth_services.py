...
class TestClientService:
    ...
    async def test_update_client_info(
        ...
        async with transaction_manager.transaction() as conn:
            ...
            updated_client = await client_service.update_client(credentials_response.client_id, update_data)
...
class TestScopeService:
    ...
    async def test_update_scope_non_admin(
        ...
        async with transaction_manager.transaction() as conn:
            ...
            with pytest.raises(HTTPException) as exc_info:
                await scope_service.update_scope(created_scope.scope_name, update_data, requesting_admin=False)
    ...
    async def test_validate_requested_scopes(
        ...
        async with transaction_manager.transaction() as conn:
            ...
            await client_repo.associate_client_scopes(created_client.id, [read_scope.id, write_scope.id])
    ...
    async def test_validate_requested_scopes_invalid(
        ...
        async with transaction_manager.transaction() as conn:
            ...
            with pytest.raises(HTTPException) as exc_info:
                await scope_service.validate_requested_scopes("nonexistent_scope", created_client.id)
    ...
    async def test_reduce_scopes_to_granted(
        ...
        async with transaction_manager.transaction() as conn:
            ...
            final_scopes = await scope_service.reduce_scopes_to_granted(requested_scopes, granted_scopes)
            ...
            final_scopes = await scope_service.reduce_scopes_to_granted(requested_scopes, invalid_granted)

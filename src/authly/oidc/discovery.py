...
class OIDCServerMetadata(BaseModel):
    ...
    issuer: str = Field(
        ...,
        description="URL using the https scheme that identifies the authorization server"
    )
    ...
    response_types_supported: List[str] = Field(
        default=["code"],
        description="List of OAuth 2.0 response types that this server supports"
    )
    ...
    scopes_supported: List[str] = Field(
        ...
        description="List of OAuth 2.0 scopes that this server supports"
    )
    ...
    claims_supported: List[str] = Field(
        default_factory=list,
        description="List of claim names supported by the UserInfo endpoint"
    )
    ...
    require_request_uri_registration: bool = Field(
        default=False,
        description="Whether request_uri values used must be pre-registered"
    )
    
    # OIDC UI locales
    ui_locales_supported: List[str] = Field(
        default=["en"],
    ...
    require_pkce: bool = Field(
        default=True,
        description="Whether PKCE is required for all clients (OAuth 2.1 requirement)"
    )
    response_modes_supported: List[str] = Field(
        default=["query", "fragment"],
        description="List of response modes supported"
    )
...
class OIDCDiscoveryService:
    ...
    async def get_oidc_server_metadata(
        self,
        issuer_url: str,

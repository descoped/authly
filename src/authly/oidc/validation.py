...
class OIDCValidator:
    ...
    def validate_oidc_scopes(requested_scopes: List[str]) -> OIDCValidationResult:
        ...
        for scope in requested_scopes:
            if scope in OIDC_SCOPES:
                ...
                if scope not in ["read", "write", "admin:clients:read", "admin:clients:write", 
                               "admin:scopes:read", "admin:scopes:write", "admin:users:read", 
                               "admin:system:read"]:
    ...
    def validate_response_type(response_type: str) -> Tuple[bool, Optional[OIDCFlow], List[str]]:
        ...
        if response_type == OIDCResponseType.CODE.value:
            ...
            flow_type = OIDCFlow.IMPLICIT
        elif response_type in [OIDCResponseType.CODE_ID_TOKEN.value, 
...
class OIDCScopeProcessor:
    ...
    def validate_scope_combination(scopes: List[str]) -> Dict[str, any]:
        ...
        known_oauth_scopes = {
            "read", "write", "admin:clients:read", "admin:clients:write",
            "admin:scopes:read", "admin:scopes:write", "admin:users:read", 
            "admin:system:read"
        }

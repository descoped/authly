...
@click.option("--redirect-uri", "redirect_uris", multiple=True, required=True, help="Redirect URI (can specify multiple)")
...
def create_client(
    ...
    if auth_method is None:
        auth_method = TokenEndpointAuthMethod.NONE if client_type == ClientType.PUBLIC else TokenEndpointAuthMethod.CLIENT_SECRET_BASIC
        ...
        async with get_api_client() as client:
            try:
                ...
                if output == "json":
                    ...
                    # Table header
                    click.echo(f"{'Client ID':<36} {'Name':<20} {'Type':<12} {'Active':<6} {'PKCE':<4} {'Redirect URIs'}")
                    click.echo("-" * 120)

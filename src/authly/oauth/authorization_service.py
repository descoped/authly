...
class AuthorizationService:
    ...
    async def generate_authorization_code(
        self, consent_request: UserConsentRequest
    ) -> Optional[str]:
        """
    ...
    async def create_authorization_error_response(
        ...
        return OAuthAuthorizationErrorResponse(
            error=error,
            error_description=description or self._get_error_description(error),
            state=state
        )
    
    async def get_requested_scopes(
        self, scope_string: Optional[str], client: OAuthClientModel
    ) -> List[str]:
        """

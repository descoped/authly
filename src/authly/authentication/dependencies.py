...
from authly.api.users_dependencies import _get_user_by_id, _validate_token_and_get_user_id
...
async def get_current_user_flexible(
    ...
    user_repo: UserRepository = Depends(get_database_connection),
    ...
    if session_cookie:
        try:
            ...
            if session_data:
                # Get full user model
                user_repo_instance = UserRepository(user_repo)
                user = await user_repo_instance.get_by_id(UUID(session_data["id"]))
    ...
    if token:
        try:
            ...
            user_repo_instance = UserRepository(user_repo)
            user = await _get_user_by_id(user_repo_instance, user_id)
            if user:
...
async def get_session_csrf_token(session_cookie: str | None = Cookie(None, alias=COOKIE_NAME)) -> str | None:
    ...
    try:
        ...
        backend = resource_manager.get_backend()
        auth_service = AuthenticationService(backend)
        session_data = await auth_service.get_user_from_session(session_cookie)
